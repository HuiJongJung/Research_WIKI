...
이 파일에 있던 계획 게이트, 한 번에 한 작업 구현 루프, journal 추가 형식, 검증 커맨드는 위키 MCP 도구를 만들던 시기의 규칙이었다. 2026-08-04에 폐기했다. `specs/`, `harness/`, `PRD.md`, `TASK.md`는 삭제했고 `journal.md`는 `archive/`로 옮겼다. 원본은 git 이력에 있다.
