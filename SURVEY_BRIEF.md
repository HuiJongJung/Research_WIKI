...
1. I_SH(또는 해당 모호도 지표)의 정의가 **높은 쪽만 잡는가, 낮은 쪽도 잡는가.** "SH 계수가 너무 크거나 너무 작은 영역을 둘 다 잡는다"는 사용자 추정이 있으나 `[미검증]`이다 — 원문으로 확정한다
2. 잡힌 구역에 대한 처방이 무엇인가 — 전부 동일하게 prior 주입인가, 구간별로 다른가
3. **낮은 쪽(정보 부족) 구역에서의 결과를 논문이 언급하는가** — ablation·limitation·정성 어디든. **언급이 없으면 그 부재 자체를 기록한다** (검색 경로와 함께)
...
- **알고리즘 재현이 목적이 아니다.** 우리 판별기("사진이 결정 못 하는 구역의 경계 긋기")의 이론적 조상으로 인용할 때 필요한 정확도가 목적이다
- 특히: **깎기가 멈추는 조건**이 곧 미결정 경계의 정의인가 — 우리 3상태 출력의 "미결정"과 photo hull 표면의 관계를 한 문단으로
