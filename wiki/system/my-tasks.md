...
- [ ] **PGSR** — ★실질 1순위 (2DGS가 보충 발췌로 축소되면서). occlusion 추정 하나로 F1 절반(0.52↔0.28)을 얻음 = **우리 가시성 채널의 "학습 중" 대응물.** 걔들 방식을 모르면 "우리는 학습 전에 한다"는 차별화를 말할 수 없다. AmbiSuR의 기반. 목표: occlusion 추정 기전 정확히
