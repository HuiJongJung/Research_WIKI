...
| ~~채널 불일치 지도~~ | 완료. z-buffer 경로 종결, 상한 서술 확정 |
| 상대 gap (angle_gap ÷ 최대 쌍각) | 신규 (CPU, 나눗셈 하나) |
| **B13 채널 스위치 배선** | **신규 (CPU 30~60분, 즉시 가능)** |
| **B13 채널 비교 5-arm** | 신규 (GPU, base2 이후) — C5 n_views가 핵심 명제의 반증 대조군 |
| ray_confidence 배선 | 보류 — 현재 4열 닫힌 뒤 별도 arm |
| clone_thr_scale 배선 | 대기 (30분) — ④′ 이진판 결과 전에 미리 |
