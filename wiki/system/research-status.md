...
**AmbiSuR 대비 (재정리)**: 그들은 출력 축(잔차/SH)으로 2분할하고 처방이 하나(prior 주입)다. 우리는 입력 축(관측 품질)으로 3분할하고 구역마다 처방이 다르다(보호/열기/표시). 그리고 "정보 없는 구역에는 주입할 정보 자체가 없다"를 S1로 실측 입증했다 — prior도 같은 사진에서 나오므로 같은 뚜껑을 지지한다(2b가 검증).
