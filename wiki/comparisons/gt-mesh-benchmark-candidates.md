...
## 6. 단일 무대 질의에 대한 추천 (2026-08-10 추가)

"복잡한 오브젝트 + GT mesh + 실내 또는 단일 물체" 조건으로 하나만 고르는 질의에 대한 조사 세션의 추천은 **MobileBrick**이다. 채택 판정은 방향 논의 세션 몫.

근거 넷.
1. GT mesh의 신뢰도가 후보 중 유일하게 **기지 기하(LEGO) 기반**이다. 스캐너 오차가 평가에 섞이지 않으며, 논문 자체가 스캐너 GT의 artifacts 문제의식으로 만들어졌다 (CVPR 2023)
2. 실사 촬영 + 단일 물체(153개, 평가용 18개) + 요철·구멍·얇은 돌출부. 합성 공신력 시비 없음
3. GS2Mesh 저장소에 MobileBrick 평가 코드가 이미 있다. 프로토콜을 발명할 필요가 없다
4. masked/unmasked 두 규약을 병기한 선례 논문(arXiv 2606.20856)이 DTU+MobileBrick에서 평가했다. Q-06의 지표 방향과 무대가 겹친다

유보 둘.
- GS 표면 재구성의 표준 무대가 아니라 SOTA 수치가 이 위에 없다. 비교군은 직접 실행해야 한다
- `gt_mesh.ply`는 GT depth의 TSDF fusion 산출물이라는 README 각주를 논문에 그대로 옮겨야 한다

차순위는 OmniObject3D(다양성 최고, 라이선스·GT 정밀도 미확인)와 Shelly(복잡 형상 최적, 공식 배포처 못 찾아 탈락). DTU 부분호 예비판과는 보완 관계다 — DTU는 호 폭 프로토콜 무대, MobileBrick은 GT mesh 정밀 평가 무대.
